...
import edu.es.eoi.marketplacespringboot.service.UsuarioServiceImpl;
...
public class UsuarioController {
	...
	public ResponseEntity<Boolean> login(@RequestBody UsuarioDto dto){
		return new ResponseEntity<Boolean>(serviceUsuario.existUser(dto), HttpStatus.OK); 
	}
	
	@GetMapping("/{id}")	
	public ResponseEntity<UsuarioDto> getOne(@PathVariable Integer id) {
		try {
			return new ResponseEntity<UsuarioDto>(serviceUsuario.find(id), HttpStatus.OK);
		}catch(Exception e) {
